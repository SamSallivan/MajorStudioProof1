...
namespace DistantLands.Cozy.Data
{
    ...
    public class ClimateProfile : ScriptableObject
    {
        ...
        public float GetTemprature(bool celsius, PerennialProfile perennialProfile)
        {


            float i = (tempratureOverYear.Evaluate(perennialProfile.YearPercentage()) * tempratureOverDay.Evaluate(perennialProfile.DayPercentage())) + tempratureFilter;
        ...
        public float GetTemprature(bool celsius, PerennialProfile perennialProfile, float inTicks)
        {

            float nextDays = inTicks / perennialProfile.ticksPerDay;

            float i = (tempratureOverYear.Evaluate((perennialProfile.dayAndTime + nextDays) / perennialProfile.daysPerYear) * tempratureOverDay.Evaluate(perennialProfile.DayPercentage())) + tempratureFilter;
        ...
        public float GetHumidity(PerennialProfile perennialProfile)
        {


            float i = (precipitationOverYear.Evaluate(perennialProfile.YearPercentage()) * precipitationOverDay.Evaluate(perennialProfile.DayPercentage())) + precipitationFilter;

            return i;
        }

        public float GetHumidity(PerennialProfile perennialProfile, float inTicks)
        {
            float nextDays = inTicks / perennialProfile.ticksPerDay;

            float i = (precipitationOverYear.Evaluate((perennialProfile.dayAndTime + nextDays) / perennialProfile.daysPerYear) * precipitationOverDay.Evaluate(perennialProfile.DayPercentage())) + precipitationFilter;
